(() => {
    ...
    for (let attempt = 0; attempt <= CONFIG.retryLimit; attempt += 1) {
      try {
        ...
        if (!response.ok) {
          ...
          const retryAfterNote = retryAfter ? ` Retry after ${retryAfter} seconds.` : "";
        ...
        const waitMs = jitter(CONFIG.retryBaseDelayMs * (attempt + 1));
        progress(`${label}: retry ${attempt + 1}/${CONFIG.retryLimit} after ${waitMs}ms`);
        await sleep(waitMs);
      }
    }
  ...
  function formatAccount(account) {
    return account.fullName ? `@${account.username} - ${account.fullName}` : `@${account.username}`;
  }
  ...
  async function loadRelationshipList(type, target) {
    ...
    for (let pass = 1; pass <= CONFIG.relationshipPasses; pass += 1) {
      for (const pageSize of CONFIG.relationshipPageSizes) {
        ...
        while (pageCount < CONFIG.maxPagesPerPass) {
          ...
          maxId = nextMaxId;
          await sleep(jitter(CONFIG.relationshipListDelayMs));
        }
        ...
        await sleep(jitter(CONFIG.relationshipListDelayMs * 2));
      }
    }
  ...
  async function exactFollowerSearch(target, username) {
    ...
    for (const url of urls) {
      const response = await getJson(url, `exact follower search @${username}`);
      const exactMatch = extractUsers(response).some(
  ...
  function statCard(label, value) {
    return `<div class="card"><span>${escapeHtml(label)}</span><strong>${formatNumber(value)}</strong></div>`;
  }
  ...
  function renderFinalReport(result) {
    ...
    document.body.innerHTML = `
      ...
      <p>Only accounts in <strong>Verified not following back</strong> are counted as misses. Tentative misses corrected by exact follower search are removed, and failures or auth issues are kept in Unknown instead of being counted.</p>
      <div class="grid">
        ...
        ${statCard("Corrected by exact search", result.correctedByExactSearch.length)}
      ...
      <h2>Corrected by exact follower search (${result.correctedByExactSearch.length})</h2>
  ...
  async function run() {
    ...
    if (authLost) {
      warnings.push("Login or rate-limit wall appeared during exact verification; affected accounts were moved to Unknown instead of counted.");
    }
    ...
    const result = {
      ...
      loaded: {
        followers: followerLoad.usersByUsername.size,
      ...
      accuracyNote: "Only accounts in Verified not following back are counted as misses. Tentative misses corrected by exact follower search are removed, and failures/auth issues are kept in Unknown instead of being counted.",
    };
    ...
    state.done = true;
    progress(
      `Done: ${verifiedNotFollowingBack.length} verified not following back, ${correctedByExactSearch.length} corrected, ${unknown.length} unknown.`,
      "done",
    );
